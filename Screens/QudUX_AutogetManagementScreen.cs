...
{
    [UIView(
        "QudUX:AutogetManagement",
        ForceFullscreen: true,
        NavCategory: "Menu,Nocancelescape",
        UICanvas: null
    )]
    public class QudUX_AutogetManagementScreen : IScreen, IWantsTextConsoleInit
    {
        ...
        public Dictionary<string, string> GetOptionList(NameValueBag AutogetSettings)
        {
            ...
            foreach (KeyValuePair<string, string> setting in AutogetSettings.Bag)
            {
                ...
                {
                    var blueprint = GameObjectFactory.Factory.GetBlueprint(
                        setting.Key.Split(':')[1]
                    );
                    ...
                    try
                    {
                        optionList.Add(displayName, setting.Key);
                    }
                    catch
                    {
                        try
                        {
                            optionList.Add(blueprint.Name, setting.Key);
                        }
                        catch
                        {
                            try
        ...
        public ScreenReturn Show(GameObject GO)
        {
            ...
            Dictionary<string, string> optionList = GetOptionList(
                QudUX_AutogetHelper.AutogetSettings
            );
            ...
            while (true)
            {
                ...
                else
                {
                    ...
                    for (
                        int i = scrollOffset;
                        i < optionStrings.Count && i < (scrollOffset + scrollAreaHeight);
                        i++
                    )
                    {
                ...
                if ((keys == Keys.Space || keys == Keys.Enter) && optionStrings.Count > 0)
                {
                    if (
                        Popup.ShowYesNo(
                            $"Remove auto-pickup exclusion for {optionStrings[selectedIndex]}?"
                        ) == DialogResult.Yes
                    )
                    {
                ...
                {
                    if (
                        Popup.ShowYesNo("Remove ALL of your auto-pickup exclusions?")
                        == DialogResult.Yes
                    )
                    {
                        string[] itemsToRemove = QudUX_AutogetHelper.AutogetSettings.Bag.Keys
                            .Where(s => s.StartsWith("ShouldAutoget:"))
                            .ToArray();
